...
A common way used to estimate epidemiological quantities is through likelihood based estimation.  We saw an example of this in the previous section with MC estimation.  However, as we will see in the results, this particular model does not seem to fit the data well.  As a result, many researchers specify different models for the generation of SIR data; estimate the likelihood through direct calculation, a method such as particle filtering, or an MCMC or other Bayesian approach; and derive point estimates and confidence intervals from this likelihood estimate.  Add in CITATIONS.
...
\begin{align*}
  s_{t} &\sim \text{Binomial}\left (\hat{X}(t), \frac{\beta Y(t)}{N} \right ) \\
  r_{t} &\sim \text{Binomial}\left (\hat{Y}(t), \gamma \right ). \nonumber
\end{align*}
