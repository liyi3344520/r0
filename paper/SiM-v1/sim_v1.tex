...
%% nice tables and sideways table
\usepackage{booktabs}
\usepackage{lscape}
...
\newcommand{\rr}{\ensuremath{\mathcal{R}_0}} %% r0 macro
...
\articletype{Statistics in Medicine}%
...
\author[1]{Shannon Gallagher*}
\author[2]{Andersen Chang}
\author[1]{William F. Eddy}



\authormark{Gallagher, Chang, and Eddy}


\address[1]{\orgdiv{Department of Statistics}, \orgname{Carnegie Mellon University}, \orgaddress{\state{PA}, \country{USA}}}
\address[1]{\orgdiv{Department of Statistics}, \orgname{Rice University}, \orgaddress{\state{TX}, \country{USA}}}


\corres{*Shannon Gallagher, Department of Statistics; Carnegie Mellon University; Pittsburgh, PA 15213. \email{sgallagh@stat.cmu.edu}}

\presentaddress{Department of Statistics; Carnegie Mellon University}
...
\keywords{Compartment models; Agent-based models; Stochastic infectious disease modelling.}

\jnlcitation{\cname{%
\author{Gallagher, S.} and
\author{Eddy, W. F}.
\ctitle{Stochastic Equivalence of Compartment and Agent-based Models}.  September 2017.}
}
...
\begin{align*}
  (\hat{\beta}_{XEYZ}, \hat{\gamma}_{XEYZ}, \hat{\mu}_{XEYZ} )&=\textnormal{arg} \textnormal{min}_{\beta, \gamma, \mu} \sum_{t} \left [ \left (X_{XEYZ}(t) - X(t)\right )^2  \right . \\
...
\begin{align*}
(\hat{\beta}_{XYZ}, \hat{\gamma}_{XYZ} )&=\textnormal{arg} \textnormal{min}_{\beta, \gamma} \sum_{t} \left [ \left (X_{XYZ}(t) - X(t)\right )^2 + \left ( Z_{XYZ}(t) - Z(t) \right )^2 \right ].
\end{align*}
Then the RE for $\rr^{XYZ}$ is given by Equation \ref{eq:sirls},
\begin{align*}
...
\begin{align*}
(\hat{\beta}, \hat{\gamma} )&=\textnormal{arg} \textnormal{min}_{\beta, \gamma} \sum_{t} \left [ \left (\hat{X}(t) - X(t)\right )^2 + \left ( \hat{Z}(t) - Z(t) \right )^2 \right ]
\end{align*}
...
\begin{align}
  ...
  \textnormal{log} \frac{X(t)}{X(0)} &=  \frac{-\beta }{\gamma}\frac{Z(t)}{N} \nonumber\\
  \textnormal{log} \frac{X(t)}{X(0)} &=  -\rr \frac{Z(t)}{N}. \label{eq:harko_lin}
\end{align}
...
\begin{align*}
  \hat{\rr} = -\frac{\sum_{t=0}^T \textnormal{log} \frac{ X(t)}{X(0)}}{\sum_{t=0}^T\frac{Z(t)}{N}}.
\end{align*}
...
\begin{align*}
  \textnormal{log} \left ( X(t+1)/ X(0) \right ) - \textnormal{log} \left ( X(t)/X(0) \right ) &= - .01\rr\\
  \textnormal{log} \left ( X(t+1) \right ) - \textnormal{log} \left ( X(t) \right )  &=- .01\rr\\
  \textnormal{log} \left ( X(t) / X(t+1) \right ) &= .01\rr\\
...
\begin{align*}
\hat{\alpha} = \textnormal{arg} \textnormal{max}_{\alpha} \mathcal{L}\left(\alpha; \textnormal{data} \right ).
  \end{align*}
 \cite{barbour2004} report that the estimated reproduction number is then,
\begin{align}\label{eq:r0-mc}
\hat{\rr} &= \textnormal{log} \left ( \frac{1}{1-\hat{\alpha}}\right ).
\end{align}
...
\begin{align}\label{eq:lbe-r0}
  (\hat{\beta}, \hat{\gamma}) &= \textnormal{arg} \textnormal{max}_{\beta, \gamma} \mathcal{L}\left ( \beta, \gamma; \textnormal{Data} \right )\nonumber\\
...
\begin{align*}
J(t+1) | \rr  \sim \textnormal{Poisson}( J(t) \textnormal{exp} \left \{  \gamma (\rr-1)\right \})
\end{align*}
...
\begin{align*}
h(\alpha) &= \textnormal{log} \left ( \frac{1}{1-\alpha}\right ) \\
  ...
  \begin{table}[H]
    \resizebox{\textwidth}{!}{%
      \begin{tabular}{@{}lllllllllll@{}}
        \toprule
        \textbf{Parameter} & $\beta$ & $\gamma$ & $\rr$ & $T$ & $X(0)$ & $Y(0)$ & $Z(0)$ & $\sigma_X$ & $\sigma_Y$ & Error Type \\ \midrule
        \textbf{Value}     & 0.06    & 0.03     & 2.00  & 365 & 99950  & 50     & 0      & 100        & 5          & Gaussian   \\ \bottomrule
      \end{tabular}%
    }
    \caption{Parameter values for baseline simulated data.}
